...
void *s21_to_upper(const char *str) {
  ...
  for (s21_size_t i = S21_SIZE_ZERO; s21_size_lt(i, len); i = s21_size_inc(i)) {
    unsigned char c = (unsigned char)str[s21_size_to_size_t(i)];

    // Если "a"-"z", то перовод в верхний регистр по ASCII
    if (c >= (unsigned char)'a' && c <= (unsigned char)'z') {
      c = (unsigned char)(c - (unsigned char)('a' - 'A'));  // Минус 32 символа
                                                            // по ASCII
    }
...
void *s21_insert(const char *src, const char *str, s21_size_t start_index) {
  ...
  for (s21_size_t i = S21_SIZE_ZERO; s21_size_lt(i, idx); i = s21_size_inc(i)) {
