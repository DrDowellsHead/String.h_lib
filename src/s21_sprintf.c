...
static void s21_emit_with_width(char **out, int *written, const s21_fmt_t *fs,
  ...
  int pad = (width > tmp_len)
                ? (width - tmp_len)
                : 0;  // сколько символов нужно заполнить до заданной ширины
  // если установлен флаг заполнения нулями и не выбрано выравнивание слева, то
  // заполняем нулями, иначе - пробелами
...
static int s21_utoa_base_rev(char *buf, unsigned long long v, int base,
  ...
  while (v != 0ULL) {
    unsigned dig =
        (unsigned)(v % (unsigned long long)base);  // получение разряда числа
    buf[len++] = d[dig];  // поиск разряда в алфавите доступных символов и
                          // запись его в буфер
    v /= (unsigned long long)
        base;  // переход к следующему разряду(более старшему)
  }
  return len;
}
...
static int s21_format_int(char *tmp, const s21_fmt_t *fs, long long sval,
  ...
  if (is_signed) {
    if (sval < 0) {
      sign_ch = '-';
      mag = (unsigned long long)(-(sval + 1)) +
            1ULL;  // для корректной обработки -2^63, иначе будет переполнение
    } else {
  ...
  // условие precission==0 строже чем precission_set, по идее можно оставить
  // только precission==0 если передали не указатель с точностью 0(вывод всех
  // символов) и само значение нулевое, то нужно вывести пустое значение за
  // исключением случая восьмеричной системы, когда 0 выведется
  ...
  if (!precission_set && fs->flag_zero && !fs->flag_minus) {
    // общая длина выводимой строки складывается из знака(если есть), префикса,
    // дополнительных(ведущих) нулей и разрядов самого числа
    int core_len = digits_len + need_zeros + prefix_len + (sign_ch ? 1 : 0);
    // ширина поля больше чем требуемая длина выводимого значения - дополняем
    // нулями справа(их будет zero_pad штук)
  ...
  for (int i = 0; i < zero_pad; i++)
    tmp[pos++] = '0';  // почему все нули выводятся слева, по идее need_zeros
                       // дожно быть слева, а zero_pad - справа
  for (int i = 0; i < need_zeros; i++) tmp[pos++] = '0';

  // записываем разряды в привычном виде - от старшего к младшему(получали
...
static int s21_format_f(char *tmp, const s21_fmt_t *fs, long double v,
  ...
  long double rounded_i = rintl(av * scale);

  // Разбиваем округлённое число на целую и дробную части
  unsigned long long int_part = (unsigned long long)(rounded_i / scale);
  unsigned long long frac_scaled =
      (unsigned long long)(rounded_i - (long double)int_part * scale);

  char int_rev[128];
  int int_len = s21_utoa_base_rev(int_rev, int_part, 10, 0);
  for (int i = int_len - 1; i >= 0; i--)
    tmp[pos++] = int_rev[i];  // записываем в буфер целую часть
...
static int s21_format_e(char *tmp, const s21_fmt_t *fs, long double v, int prec,
  ...
  // 2) Округление мантиссы целиком (не только дроби!)
  long double scale = s21_pow10_ld(prec);

  // banker’s rounding (ties-to-even) для mantissa
  // mant_scaled — это целое число, представляющее mant * 10^prec
  unsigned long long mant_scaled = (unsigned long long)rintl(mant * scale);

  // перенос, если округление дало 10.0000...
  // Например 9.9999 -> округлили -> 10.0000 => mant=1.0000, exp++
  unsigned long long limit = (unsigned long long)(10.0L * scale);
  if (mant_scaled >= limit) {
    mant_scaled /= 10ULL;
    exp10++;
  }

  // 3) Печать: 1 цифра до точки + prec цифр после точки
  unsigned long long scale_i = (unsigned long long)scale;
  unsigned long long first =
      (scale_i == 0ULL) ? mant_scaled : (mant_scaled / scale_i);
  unsigned long long frac_scaled =
      (scale_i == 0ULL) ? 0ULL : (mant_scaled % scale_i);

  tmp[pos++] = (char)('0' + (int)first);
  ...
  // формат экспоненты как у libc: минимум 2 цифры, сотни — только если >=100
  if (eabs >= 100) {
    tmp[pos++] = (char)('0' + (eabs / 100));
  }
  tmp[pos++] = (char)('0' + (eabs / 10) % 10);
  tmp[pos++] = (char)('0' + (eabs % 10));
...
static int s21_vsprintf(char *str, const char *format, va_list ap_src) {
  ...
  while (*f != '\0') {
    ...
    if (fs.spec == 'p') {
      ...
      s21_fmt_t fs2 = fs;
      fs2.flag_hash = 0;  // запрещаем указателю распечатываться как-то иначе,
                          // чем в 16-чной системе счисления
      fs2.spec = 'x';
