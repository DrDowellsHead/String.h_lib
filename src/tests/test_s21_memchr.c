...
void compare_memchr_results(const void *str, int c, size_t n,
  ...
  if (std_result == NULL) {
    ...
    ck_assert_msg(std_result == our_result,
                  "%s: Pointers differ. Standard: %p, Our: %p", test_name,
                  std_result, our_result);
    ck_assert_msg(*(char *)std_result == *(char *)our_result,
                  "%s: Values differ. Standard: %d, Our: %d", test_name,
                  *(char *)std_result, *(char *)our_result);
  }
}
...
void assert_both_null(const void *str, int c, size_t n, const char *test_name) {
  ...
  ck_assert_msg(our_result == NULL,
                "%s: s21_memchr should return NULL but returned %p", test_name,
                our_result);
}
...
START_TEST(test_negative_char_standard) {
  ...
  compare_memchr_results(str, (int)(unsigned char)0xFF, sizeof(str),
                         "negative_char_2");
